  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto w-full">
      ...
      >
        ...
        <p className="text-sm md:text-base text-slate-400 mt-1">Your tax filing overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        {/* Category breakdown */}
        <div
          className="card p-4 md:p-6 lg:col-span-2 opacity-0 animate-fade-up"
          style={{ animationDelay: '240ms', animationFillMode: 'forwards' }}
        >
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div className="flex items-center gap-2">
              <BarChart3 className="w-4 md:w-5 h-4 md:h-5 text-brand-400" />
              <h2 className="text-sm md:text-base font-semibold text-white">Spending by Category</h2>
            </div>
          </div>
          {topCats.length === 0 ? (
              ...
              {topCats.map(([cat, amt]) => (
                <div key={cat}>
                  <div className="flex justify-between text-xs md:text-sm mb-1">
                    ...
                    <span className="text-slate-400 font-mono ml-2">{fmt(amt)}</span>
                  </div>
        ...
        {/* Recent Bills */}
        <div
          className="card p-4 md:p-6 opacity-0 animate-fade-up"
          style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}
        >
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div className="flex items-center gap-2">
              <Receipt className="w-4 md:w-5 h-4 md:h-5 text-brand-400" />
              <h2 className="text-sm md:text-base font-semibold text-white">Recent Bills</h2>
            </div>
          ...
          {recentExpenses.length === 0 ? (
            <div className="text-center py-6 md:py-8">
              ...
              <Link href="/upload" className="btn-primary mt-3 md:mt-4 inline-flex text-xs md:text-sm">
                Upload a bill
              </Link>
            </div>
          ) : (
              ...
              {recentExpenses.map(exp => (
                ...
                >
                  ...
                  <div className="text-right ml-2">
                    <p className="text-xs md:text-sm font-mono font-semibold text-white">{fmt(exp.amount)}</p>
                    <span className={clsx(
                      'text-xs',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          {[
            ...
            <Link
              key={href}
              href={href}
              className="card p-4 md:p-5 flex items-start gap-3 md:gap-4 hover:border-brand-500/50 hover:bg-brand-500/5 transition-all group"
            >
              <span className="text-lg md:text-2xl">{icon}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm md:text-base font-semibold text-white group-hover:text-brand-300 transition-colors line-clamp-2">{label}</p>
                <p className="text-xs md:text-sm text-slate-500 line-clamp-2">{desc}</p>
              </div>
              <ArrowUpRight className="w-3 md:w-4 h-3 md:h-4 text-slate-600 group-hover:text-brand-400 mt-1 shrink-0 transition-colors" />
            </Link>
          ))}
        </div>
      </div>
